...
//needs the states which are declared in same function here data is kept out due to it is constant only changes one must be in array of same scope
  ...
  <pre>
{`Note:-
const sameArr = Array(5).fill([])    //[[], [], [], [], []]  - all have same reference
const differentArr = Array.from({length: 5}, (_,i)=> 0)  //[[], [], [], [], []]

1. Array.from()

// Numbers 1 → 5
const arr = Array.from({ length: 5 }, (_, i) => i + 1);    // each array has different reference
...
console.log(users);     //[{1, user1}, {2, User2}, {3, user3}]
...
const arr5 = new Array(5).fill(0).map((_, i) => i + 1);

console.log(arr5);   // [1, 2, 3, 4, 5]

// Objects
const products = new Array(3).fill(0).map((_, i) => ({ id: i + 1}));
...
❌ Wrong
const wrong = Array(3).fill([]);   // All rows share same reference
✅ Correct
