...
const ToDoListWithoutIndex= ()=>{
    ...
    const removeTask = (removeTask)=>{
        setTaskList(prev=>prev?.filter((availTasks)=>availTasks !== removeTask))
    }
...
Add todo: setItem([...items, newItem])
delete:   setItem(prev=> prev.filter(res=> res.id !== deleteId))
update:   setItem(prev=> prev.map(res=> res.id===updatedId? updatedValue: res))
...
const handleDrop = (dropIndex)=>{
    ...
    copy.splice(dragIndex (deleteBeginIndex),1 (no.of items to delete from begin index));   // a   b   d   e   f     ---- Removed it from it's position
