...
function DragAndDropTodoDemo() {
    ...
    const deleteTask = (removeIndex) => {
        setTaskList((prev) => prev.filter((_, index) => index !== removeIndex)
        );
    };

    const updateTask = () => {
        if (!updateValue.trim()) return;
            setTaskList((prev) => prev.map((value, index) => updateIndex === index ? updateValue: value)
        );
        setUpdateIndex(null);
        setUpdateValue('');
    };
...
<>
    ...
    {taskList.map((item, index) => (
        index === updateIndex ? (
            ...
            <div key={index} draggable tabIndex={0} onDragOver={(e)=>e.preventDefault()}
                onDragStart={(event) => { dragRef.current = {index, element: event.currentTarget}; }}
...
export default DragAndDropTodoDemo;